...
class Test(object):
	...
	def apply_template(self,variable_name,context):
		val = None
	...
	def get_url(self, context=None):
		...
		val = self.apply_template(self.NAME_URL, context)
		if val is None:
			val = self._url
		return val
	...
	def parse_test(cls,base_url,node,input_test=None,test_path=None):
		...
		def use_config_parser(configobject, configelement, configvalue):
			...
			if myparsing:
				...
				setattr(configobject,configelement,configvalue)
				return True
			return False
